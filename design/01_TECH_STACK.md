...
| 원천/타깃 | Oracle | free(로컬) / thick(실타깃) | `gvenzl/oracle-free` 로컬 |
| 관측성 | Oracle CTL 스키마 | - | 자체 메트릭(PCS_CTL) |
| 관측성 | OpenLineage → Marquez | - | 계보 |
| 관측성 | OpenMetadata | - | 카탈로그/데이터 프로덕트 |
...
## Oracle 드라이버
- 로컬 thin, 실타깃 **thick**(버전 상이). → `OracleConnector`([06_COMMON_LAYER.md](06_COMMON_LAYER.md))
