...
| 계층 | 어디서 | 무엇 |
|---|---|---|
| CTL | `PCS_CTL` 스키마 | 실행이력(`C_JOB_RUN`)·워터마크(`C_SOURCE_WATERMARK`)·DQ 결과(`A_DQ_RESULT`) |
| Marquez | http://localhost:3000 (ns `pcs`) | 기술 계보 + 런타임(느린/끊긴 단계) |
| OpenMetadata | http://localhost:8585 | 비즈니스 카탈로그(도메인·용어·테이블) |
...
3. **이상 발견 → 지시서**: 원인 단계는 Marquez로 추적하고, 조치는 `../workspace/instructions/`에 지시서로 남긴다(구두/채팅 금지).
