...
## 규칙
1. **정의 없이 구현 없다 (slv/gold)**: 운영 계층 dbt 모델은 glossary 항목(산식·단위·결측 정책 확정) 이후에만 작성한다. 예외: `wrk` 실험 계층은 정의 없이 저작 가능 — slv/gold 승격 시 확정 (→ [design/07](../design/07_AUTHORING_FLOW.md)).
